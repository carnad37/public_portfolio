  ...
  <div class="d-flex container-fluid h-100">
    ...
    <div class="card align-self-center w-100 border-0 shadow">
      ...
      <div class="form-group mb-0">
		...
		<form action="dhdmew" method="POST">
...
<script>
	...
	function payment() {
        ...
        IMP.init('imp72080680'); // 'iamport' 대신 부여받은 "가맹점 식별코드"를 사용
	...
	function makeList() {
		...
		$.ajax({
			type : "GET",
			url : "<c:url value="/profile/buy_data"/>",
			data : {"page": nowPage},
            dataType : 'json',
			...
			success : function(data) {
				...
				for (let index = 0; index < data.length; index++) {		
					...
					if (data[index].o_state == 1) {
						$tr.children('td').eq(2).append($('<button type="button" class="btn btn-primary" onclick="payment()"><small>입금대기</small></button>'));		
